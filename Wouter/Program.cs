using System.Data.SqlClient;  // Importeer de benodigde namespace voor het werken met SQL Server databases.
using System.Xml.Linq;        // Importeer de benodigde namespace voor het werken met XML, hoewel deze in deze code niet gebruikt wordt.
...
namespace Wouter  // Definieer een namespace genaamd 'Wouter' om de code logisch te groeperen.
{
    class Program  // Definieer een klasse genaamd 'Program'.
    {
        ...
        {
            using (SqlConnection connection = new SqlConnection(connectionString))  // Maak een nieuwe verbinding met de database.
            {
                ...
                using (SqlCommand command = new SqlCommand(selectQuery, connection))  // Maak een nieuw SQL-commando.
                {
        ...
        static void CreateDatabaseAndTable()  // Methode om de database en tabel te creëren.
        {
            ...
            {
                connection.Open();  // Open de verbinding.
                string createDatabaseQuery = $"CREATE DATABASE {databaseName}";  // SQL-query om de database te creëren.
                using (SqlCommand command = new SqlCommand(createDatabaseQuery, connection))  // Maak een nieuw SQL-commando.
                {
                ...
                CREATE TABLE {tableName} (
                    Id INT PRIMARY KEY IDENTITY,  // Primaire sleutel met automatische nummering.
                    Name NVARCHAR(50),  // Kolom voor de naam.
                    Age INT  // Kolom voor de leeftijd.
                );";

                using (SqlCommand command = new SqlCommand(createTableQuery, connection))  // Maak een nieuw SQL-commando.
                {
                    command.ExecuteNonQuery();  // Voer de query uit.
        ...
        static void AddData(string name, int age)  // Methode om gegevens toe te voegen aan de tabel.
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))  // Maak een nieuwe verbinding met de database.
                {
                    ...
                    using (SqlCommand command = new SqlCommand(insertQuery, connection))  // Maak een nieuw SQL-commando.
                    {
        ...
        static void UpdateData(int id, string name, int age)  // Methode om gegevens in de tabel bij te werken.
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))  // Maak een nieuwe verbinding met de database.
                {
                    ...
                    using (SqlCommand command = new SqlCommand(updateQuery, connection))  // Maak een nieuw SQL-commando.
                    {
        ...
        static void DeleteData(int id)  // Methode om gegevens uit de tabel te verwijderen.
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))  // Maak een nieuwe verbinding met de database.
                {
                    ...
                    using (SqlCommand command = new SqlCommand(deleteQuery, connection))  // Maak een nieuw SQL-commando.
                    {
                        command.Parameters.AddWithValue("@Id", id);  // Voeg de parameter voor id toe.
                        command.ExecuteNonQuery();  // Voer de query uit.
